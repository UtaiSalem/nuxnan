...
class AcademyPointWithdrawalTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function makeSetup(int $balance = 100000): array
    {
        ...
        $account = AcademyPointAccount::create([
    ...
    private function svc(): AcademyPointWithdrawalService
    {
        return app(AcademyPointWithdrawalService::class);
    }

    public function test_request_creates_pending_and_reserves_balance(): void
    {
        ['owner' => $o, 'Academy' => $c, 'account' => $a] = $this->makeSetup();
        $r = $this->svc()->request($o, $c, 25000, 'buy books', null);
        $this->assertSame(AcademyPointWithdrawalRequest::STATUS_PENDING, $r->status);
        $this->assertSame(25000, (int) $a->fresh()->reserved_balance);
        $this->assertSame(1, AcademyPointTransaction::where('type', 'withdrawal_reserve')->count());
    }
    ...
    {
        ['owner' => $o, 'Academy' => $c] = $this->makeSetup(balance: 200000);
    ...
    public function test_reject_releases_reserve(): void
    {
        ...
        $this->assertSame(1, AcademyPointTransaction::where('type', 'withdrawal_release')->count());
    }

    public function test_mark_paid_debits_balance_and_credits_requester_pp(): void
    {
        config(['wallet.Academy_withdraw.maker_checker_threshold' => 5000]);
        ['owner' => $o, 'Academy' => $c, 'account' => $a] = $this->makeSetup(balance: 200000);
        $r = $this->svc()->request($o, $c, 30000, null, null);
