...
class CourseActivityController extends Controller implements HasMiddleware
{
    ...
    public function index(Course $course)
    {
        ...
        try {
            ...
            $course->loadMissing(['user', 'academy', 'courseSettings', 'courseLessons' => function ($query) {
                $query->select('id', 'course_id', 'title', 'order', 'status', 'min_read')
                    ->withCount('topics')
                    ->with([
                        'topics' => function ($q) {
                            $q->select('id', 'lesson_id', 'title', 'status')
                                ->orderBy('id');
                        },
                        'progress' => function ($q) {
            ...
            $course->loadCount(['courseLessons', 'courseMembers', 'courseAssignments', 'courseQuizzes']);
            ...
            \Log::error('CourseActivityController@index: '.$e->getMessage());

            return response()->json([
                'success' => false,
                'message' => 'เกิดข้อผิดพลาดในการโหลดข้อมูลรายวิชา',
