...
class UpdateRevenueSharePolicyRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        if ($this->route('policy')) {
            $p = $this->route('policy');
            $this->merge(array_merge(['scope_type' => $p->scope_type, 'scope_id' => $p->scope_id, 'student_pct' => $p->student_pct, 'course_pct' => $p->course_pct, 'platform_pct' => $p->platform_pct, 'effective_from' => $p->effective_from, 'effective_to' => $p->effective_to, 'notes' => $p->notes], $this->all()));
        }
    }
    ...
    public function withValidator(Validator $v): void
    {
        $v->after(function (Validator $v) {
            ...
            } if ($v->errors()->isEmpty() && round((float) $this->student_pct + (float) $this->course_pct + (float) $this->platform_pct, 2) !== 100.0) {
