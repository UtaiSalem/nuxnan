...
### 1. การโพสต์ (Posting)
- **โพสต์ทั่วไป**: +180 แต้ม
- **โพสต์พร้อมรูปภาพ**: +240 แต้ม
- **โพสต์พร้อมโพล**: +180 แต้ม + แต้มใน pool
- **โพสต์ในคอร์ส**: +180 แต้ม

### 2. การโต้ตอบ (Interactions)
- **กดไลค์โพสต์**: -24 แต้ม (ผู้กด) / +12 แต้ม (เจ้าของ)
- **ยกเลิกไลค์**: -12 แต้ม (ผู้กด)
- **กดดิสไลค์**: -12 แต้ม (ผู้กด) / -12 แต้ม (เจ้าของ)
- **แชร์โพสต์**: -36 แต้ม (ผู้แชร์) / +18 แต้ม (เจ้าของ)
- **คอมเมนต์**: -12 แต้ม (ผู้คอมเมนต์) / +6 แต้ม (เจ้าของ)

### 3. การเรียนรู้ (Learning)
- **เข้าเรียนบทเรียน**: +10-50 แต้ม (ขึ้นกับระดับ)
- **ทำแบบฝึกหัดถูก**: +คะแนนที่ได้
- **ส่งงาน**: +5-20 แต้ม
- **ผ่านแบบทดสอบ**: +50-200 แต้ม
- **เข้าเรียนต่อเนื่อง**: +20-100 แต้ม/วัน (Streak bonus)

### 4. การโหวต (Polling)
- **โหวต**: +points_per_vote แต้ม (จาก pool)
- **สร้างโพล**: -180 แต้ม + points_pool

### 5. การดูโฆษณา (Ad Viewing)
- **ดูโฆษณา**: +4% ของ duration เป็นเงิน wallet
- **ค่าใช้จ่าย**: 20 แต้ม × duration

### 6. การบริจาค (Donations)
- **รับบริจาค**: +240 แต้ม/ครั้ง
- **บริจาค**: -270 แต้ม/ครั้ง

### 7. การสนับสนุน (Support)
- **สนับสนุนโฆษณา**: แลกเปลี่ยนเงิน → แต้ม (1 บาท = 684 แต้ม)

### 8. โบนัสพิเศษ (Special Bonuses)
- **Daily Login**: +10-50 แต้ม/วัน
- **Weekly Login**: +100-500 แต้ม/สัปดาห์
- **Monthly Login**: +500-2000 แต้ม/เดือน
- **Referral**: +500-1000 แต้ม/คน
- **Birthday**: +1000 แต้ม
- **Level Up**: +100 × level แต้ม
...
### 2. การเข้าถึงเนื้อหา (Content Access)
...
- **โพสต์พร้อมรูปภาพ**: 240 แต้ม
- **สร้างโพล**: 180 แต้ม + points_pool
- **สร้างคำถาม**: 1-100 แต้ม

### 4. การแลกรางวัล (Reward Redemption)
- **แลกเงิน**: 1,080 แต้ม = 1 บาท
...
- **แปลงเป็นเงิน**: 1,080 แต้ม = 1 บาท
- **แปลงเป็น wallet**: 1,080 แต้ม = 1 บาท
...
- `post_like_received`: รับไลค์
- `post_share_received`: รับการแชร์
- `comment_received`: รับคอมเมนต์
- `quiz_complete`: ทำแบบทดสอบ
- `assignment_submit`: ส่งงาน
- `daily_login`: เข้าสู่ระบบประจำวัน
- `streak_bonus`: โบนัสการเข้าต่อเนื่อง
- `achievement_complete`: บรรลุความสำเร็จ
...
#### 2. Spend (การใช้แต้ม)
- `post_like`: กดไลค์
- `post_unlike`: ยกเลิกไลค์
- `post_dislike`: กดดิสไลค์
- `post_share`: แชร์
- `comment_create`: คอมเมนต์
- `reply_create`: ตอบคอมเมนต์
- `lesson_access`: เข้าบทเรียน
- `course_create`: สร้างคอร์ส
- `poll_create`: สร้างโพล
- `reward_redeem`: แลกรางวัล
- `donation_give`: บริจาค
...
#### 4. Transfer (การโอนแต้ม)
- `user_transfer`: โอนแต้มให้ผู้ใช้อื่น
- `admin_transfer`: โอนแต้มจาก admin
...
- `wallet_to_points`: แปลงเงินเป็นแต้ม
  ...
  <div class="conversion-modal">
    ...
    <div class="conversion-info">
      <div class="exchange-rate">
        ...
        <span class="rate-value">1,080 แต้ม = 1 บาท</span>
      </div>
...
1. **ระบบแต้มสะสม (Points System)**: จัดการแต้มที่ได้และใช้
2. **ระบบ Wallet**: จัดการเงินและการทำธุรกรรม
