...
// Check if user is not a member of course (support multiple legacy fields)
const isCourseMember = computed(() => {
  if (props.isCourseAdmin) return true;

  const courseData = props.course?.data || {};
  const member = props.courseMemberOfAuth;
  const memberStatus = member?.course_member_status ?? member?.status;

  return (
    courseData?.isMember === true ||
        memberStatus === 1 || memberStatus === '1' ||
        courseData?.member_status === 1 || courseData?.member_status === '1'
  );
});

const isNotMember = computed(() => !isCourseMember.value);
...
<template>
    <MainLayout :title="props.course.data.name">
        ...
        <template #mainContent>
            <div class="mt-4">
                <div
                    v-if="!props.isCourseAdmin && isNotMember"
                    class="mb-4 rounded-lg border border-yellow-200 bg-yellow-50 px-4 py-3 text-yellow-800"
                >
                    <div class="font-semibold">คุณยังไม่เป็นสมาชิกของรายวิชานี้</div>
                  <div class="text-sm">กรุณากด “ขอเป็นสมาชิก” ด้านบน หรือกดเข้าร่วมกลุ่มที่เมนูกลุ่ม</div>
                </div>
