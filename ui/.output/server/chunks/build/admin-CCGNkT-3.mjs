import { defineComponent, inject, ref, mergeProps, unref, computed, useSSRContext } from 'vue';
...
import { p as useRoute, i as useApi, d as useAuthStore } from './server.mjs';
import '../_/nitro.mjs';
import 'node:http';
import 'node:https';
import 'node:events';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  props: {
    ...
    status: {
      type: String,
      default: null,
      validator: (value) => [null, "online", "offline", "busy"].includes(value)
    }
  },
  setup(__props) {
    const props = __props;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative inline-block" }, _attrs))}><img${ssrRenderAttr("src", avatarSrc.value)}${ssrRenderAttr("alt", __props.alt)} class="${ssrRenderClass([sizeClasses.value, "rounded-full object-cover border-2 border-white dark:border-gray-800"])}">`);
      if (__props.status) {
        _push(`<span class="${ssrRenderClass([statusColor.value, "absolute bottom-0 right-0 block w-3 h-3 rounded-full ring-2 ring-white dark:ring-gray-800"])}"></span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
_sfc_main$1.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/atoms/BaseAvatar.vue");
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getAdminAvatar = (admin) => {
      var _a, _b, _c, _d;
      if ((_a = admin.user) == null ? void 0 : _a.avatar) return admin.user.avatar;
      if (((_b = authStore.user) == null ? void 0 : _b.id) === ((_c = admin.user) == null ? void 0 : _c.id)) return (_d = authStore.user) == null ? void 0 : _d.avatar;
      return null;
    };
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(showInviteModal)) {
        ...
        if (unref(isSearching)) {
          ...
          ssrRenderList(unref(searchResults), (user) => {
            _push(`<div class="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded shadow-sm hover:shadow-md transition-shadow"><div class="flex items-center gap-3">`);
            _push(ssrRenderComponent(_sfc_main$1, {
              src: user.avatar,
              name: user.name,
              size: "md"
            }, null, _parent));
            _push(`<div><p class="font-medium text-sm">${ssrInterpolate(user.name)}</p><p class="text-xs text-gray-500">${ssrInterpolate(user.email)}</p></div></div><button class="text-blue-600 hover:text-blue-800 text-sm font-medium px-3 py-1 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors"> \u0E40\u0E0A\u0E34\u0E0D </button></div>`);
          });
          _push(`<!--]--></div>`);
...
//# sourceMappingURL=admin-CCGNkT-3.mjs.map
