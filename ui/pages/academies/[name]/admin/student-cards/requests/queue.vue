...
<template><div class="space-y-6"><div><h1 class="text-2xl font-bold dark:text-white">คิวจัดทำบัตร</h1><p class="text-gray-500">อนุมัติและติดตามคำร้องทั้งหมด</p></div>
  ...
  <div class="overflow-hidden rounded-xl border bg-white dark:border-gray-700 dark:bg-gray-800"><div v-if="loading" class="p-12 text-center">กำลังโหลด...</div><table v-else class="w-full text-left"><thead class="bg-gray-50 dark:bg-gray-900"><tr><th class="p-4">นักเรียน</th><th class="p-4">ชั้น/ห้อง</th><th class="p-4">ครูประจำชั้น</th><th class="p-4">ประเภท</th><th class="p-4">สถานะ</th><th class="p-4"></th></tr></thead><tbody><tr v-for="row in rows" :key="row.id" class="border-t dark:border-gray-700"><td class="p-4"><div class="font-medium dark:text-white">{{ row.full_name }}</div><div class="text-xs text-gray-500">{{ row.student_number }}</div></td><td class="p-4">{{ row.grade_level }}/{{ row.section }}</td><td class="p-4">{{ row.classroom?.homeroom_teacher?.name || '—' }}</td><td class="p-4">{{ row.request_type }}</td><td class="p-4"><SchoolStudentCardRequestStatusBadge :status="row.status" /></td><td class="p-4 text-right"><NuxtLink :to="`/academies/${academyName}/admin/student-cards/requests/${row.id}`" class="text-primary-600">ดูรายละเอียด</NuxtLink></td></tr><tr v-if="!rows.length"><td colspan="6" class="p-10 text-center text-gray-500">ไม่มีคำร้องในสถานะนี้</td></tr></tbody></table></div>
</div></template>
