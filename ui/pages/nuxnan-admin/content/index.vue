...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-800 dark:text-white">จัดการเนื้อหา</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1">จัดการโพสต์ บทความ และเนื้อหาต่างๆ</p>
      </div>
      <NuxtLink
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-slate-50 dark:bg-slate-700/50">
            <tr>
              <th class="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase">หัวข้อ</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase">ประเภท</th>
              <th class="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase">ผู้เขียน</th>
              <th class="px-6 py-4 text-right text-xs font-medium text-slate-500 uppercase">ยอดดู</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
            <tr v-for="content in contents" :key="content.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
              <td class="px-6 py-4">
                <p class="font-medium text-slate-800 dark:text-white line-clamp-1">{{ content.title }}</p>
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ content.author }}</td>
              <td class="px-6 py-4 text-right text-slate-600 dark:text-slate-300">{{ content.views.toLocaleString() }}</td>
              ...
              <td class="px-6 py-4 text-right text-slate-500">{{ content.date }}</td>
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button class="min-h-[44px] sm:min-h-0 min-w-[44px] sm:min-w-0 inline-flex items-center justify-center p-2 text-slate-500 hover:text-hopeui-primary-600 hover:bg-hopeui-primary-100 dark:hover:bg-hopeui-primary-900/30 rounded-lg transition-colors">
                    <Icon icon="fluent:eye-24-regular" class="w-5 h-5" />
                  </button>
                  <button class="min-h-[44px] sm:min-h-0 min-w-[44px] sm:min-w-0 inline-flex items-center justify-center p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors">
