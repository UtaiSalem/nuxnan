...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-800 dark:text-white">จัดการอะคาเดมี</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1">จัดการอะคาเดมีทั้งหมดในระบบ</p>
      </div>
      <NuxtLink
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-hopeui border border-slate-100 dark:border-slate-700 overflow-hidden">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
        <div
          ...
          :key="academy.id"
          class="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 hover:shadow-md transition-shadow"
        >
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <h3 class="font-semibold text-slate-800 dark:text-white line-clamp-1">{{ academy.name }}</h3>
              <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mt-1">{{ academy.description }}</p>
        ...
        <div v-if="academies.length === 0" class="col-span-full p-8 text-center">
          <Icon icon="fluent:building-24-regular" class="w-12 h-12 text-slate-300 mx-auto" />
          <p class="text-slate-500 mt-2">ไม่พบอะคาเดมี</p>
        </div>
      </div>
    </div>
  </div>
