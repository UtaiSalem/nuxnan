...
const joinCode = ref('')
const view = ref<'home' | 'lobby' | 'playing' | 'result'>('home')
...
<template>
  <NuxtLayout name="main">
    <div class="max-w-4xl mx-auto py-12 px-4">
      ...
      <div v-if="view === 'home'" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div class="grid md:grid-cols-2 gap-6">
          ...
          <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 space-y-6 shadow-xl shadow-slate-200/50 dark:shadow-none transition-transform hover:scale-[1.02]">
            ...
            <button @click="handleCreate" class="w-full py-4 bg-primary-600 hover:bg-primary-700 text-white font-black text-lg rounded-2xl transition-all shadow-lg shadow-primary-500/30">
              🚀 สร้างห้องใหม่
            </button>
          </div>
          ...
          <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 space-y-6 shadow-xl shadow-slate-200/50 dark:shadow-none transition-transform hover:scale-[1.02]">
            ...
            />
            <button @click="handleJoin" class="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-lg rounded-2xl transition-all shadow-lg shadow-slate-400/20">
              🎯 เข้าร่วม
            </button>
          </div>
        </div>
      ...
      <!-- COUNTDOWN -->
      <div v-else-if="raceStatus === 'countdown'" class="flex flex-col items-center justify-center min-h-[60vh] animate-in zoom-in duration-300">
        <div class="text-center">
