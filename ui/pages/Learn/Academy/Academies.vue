      ...
      <template #mainContent>
        ...
        <div class="bg-white shadow-xl w-full max-w-7xl mx-auto rounded-lg overflow-hidden mt-4">
          <div class="flex flex-row justify-around">
            <button
              @click.prevent="handleGetAcademies(0)"
            ...
            >
              <div class="flex flex-col items-center py-2 justify-center text-slate-700">
                <Icon
                  ...
                  class="w-6 h-6"
                  :class="{ 'text-cyan-500': academiesTypeIndex === 0 }"
                />
                <span :class="{ 'text-cyan-500': academiesTypeIndex === 0 }">{{
                  academiesType[0].title
                }}</span>
              </div>
            </button>
            <button
              @click.prevent="handleGetAcademies(1)"
              class="border-b-4 hover:border-gray-400 rounded-none w-full text-center flex-row justify-center"
            ...
            >
              <div class="flex flex-col items-center py-2 justify-center text-slate-700">
                <Icon
                  icon="lucide:layout-list"
                  class="w-6 h-6"
                  :class="{ 'text-cyan-500': academiesTypeIndex === 1 }"
                />
                <span :class="{ 'text-cyan-500': academiesTypeIndex === 1 }">{{
                  academiesType[1].title
                }}</span>
              </div>
            </button>
            <button
              @click.prevent="handleGetAcademies(2)"
              class="border-b-4 hover:border-gray-400 rounded-none w-full text-center flex-row justify-center"
            ...
            >
              <div class="flex flex-col items-center py-2 justify-center text-slate-700">
                <Icon
                  icon="lucide:layout-list"
                  class="w-6 h-6"
                  :class="{ 'text-cyan-500': academiesTypeIndex === 2 }"
                />
                <span :class="{ 'text-cyan-500': academiesTypeIndex === 2 }">{{
                  academiesType[2].title
                }}</span>
              </div>
            </button>
          </div>
        ...
        <div class="section-header my-4 p-4 bg-white rounded-xl shadow-lg">
          <div class="flex justify-between">
            <h2 class="font-bold text-2xl text-gray-700 font-prompt">
              {{ academiesType[academiesTypeIndex].title }}
              {{ ' (' + academies.length + ')' || 'ยังไม่มีแหล่งเรียนรู้' }}
            </h2>
          </div>
        </div>
        ...
        >
          <p class="text-gray-800 dark:text-gray-200">
            {{ academiesType[academiesTypeIndex].errorMessages }}
          </p>
        </div>
      </template>
    </MainLayout>
