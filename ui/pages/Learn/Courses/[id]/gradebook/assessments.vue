...
/**
 * Course Gradebook - Score Entry (Legacy Mode)
 */
...
onMounted(async () => {
  if (courseId.value) {
    // Legacy Check
    if (!course.value?.use_legacy_gradebook) {
       navigateTo(`/Learn/Courses/${courseId.value}/gradebook`)
       return
    }
    await fetchData()
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
          <Icon icon="fluent:clipboard-edit-outline" class="w-7 h-7 text-primary-500" />
          การประเมิน (Legacy Mode)
        </h1>
