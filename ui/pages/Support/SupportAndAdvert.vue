...
const browseInputMedia = () => {
  inputMediaImage.value.click()
}
...
function computeQuantityToShowProductMedia(mIdx = 0, tIdx = 0) {
  ...
  quantityToShowProductMedia.value = Math.floor(
    (totalMoneySupport.value * 684) / (timeToShowProductMedia.value * 64)
  )
  bonus.value =
    totalMoneySupport.value >= 50 && timeToShowProductMedia.value >= 5
      ? quantityToShowProductMedia.value + timeToShowProductMedia.value
...
async function submitForm(e) {
  ...
  try {
    ...
    if (supportResp.data && supportResp.data.success) {
      Swal.fire('เสร็จสมบูรณ์', 'เพิ่มแหล่งเรียนรู้ใหม่เรียบร้อยแล้ว', 'success')
      // router.get(`/academies/${resp.data.academy}`)
    } else {
      Swal.fire('เกิดข้อผิดพลาด', supportResp.data.message, 'error')
    }
  } catch (error) {
  ...
  <div>
    <MainLayout title="Support Plearnd.com">
      <template #coverProfileCard>
        <div
          class="flex items-center max-w-7xl mx-auto jusb mt-2 mb-4 shadow-lg bg-cover bg-no-repeat rounded-lg overflow-hidden"
      ...
      <template #mainContent>
        <div class="plearnd-card">
          ...
          <h3 class="text-lg font-semibold text-gray-600 dark:text-white py-2">ผู้สนับสนุน</h3>
          ...
          >
            <div class="grid grid-cols-1 gap-4 mt-4">
              <div>
                ...
                <select
                  ...
                  @change.prevent="
                    computeQuantityToShowProductMedia(moneyIndexSelected, timeIndexSelected)
                  "
            ...
            <div class="grid grid-cols-1 gap-4 mt-4">
              <div>
                ...
                <select
                  ...
                  @change.prevent="computeQuantityToShowProductMedia(0, timeIndexSelected)"
                  class="w-full text-center rounded-lg border border-gray-400 py-2 px-3 dark:bg-gray-700 dark:text-white dark:border-none"
                >
                  <!-- <option value="5" selected>5</option> -->
                  <option
                    v-for="(timeOption, tidx) in timeToShowProductMediaOptions"
                    :key="tidx"
                    :value="tidx"
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
              <p class="block text-gray-700 dark:text-white mb-1">วัน/เวลา ที่ทำรายการ</p>
            </div>
