  ...
  <div>
    <MainLayout title="ทุนสนับสนุนการเรียนรู้">
      ...
      <template #mainContent>
        ...
        >
          <button
            class="flex items-center justify-center max-w-fit px-4 py-2 my-1 font-medium text-center text-white rounded-md bg-teal-400"
        ...
        <template v-if="props.adverts.data.length">
          <div class="grid md:grid-cols-2 gap-4">
            <div
              v-for="(advert, index) in refAdvertsData"
              :key="index"
              class="border mb-2 rounded-lg hover:shadow-indigo-300 hover:shadow-md bg-white"
            >
              <AdvertiseItemCard :advert="advert" :index="index" />
            </div>
          </div>
        </template>
        <template v-else>
