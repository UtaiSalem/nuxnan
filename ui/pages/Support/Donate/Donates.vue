...
const handleGetDonateSuccess = () => {
  if (usePage().props.auth.user) {
    usePage().props.auth.user.pp += 240
  }
  ...
  <div>
    <MainLayout title="ทุนสนับสนุนการเรียนรู้">
      <template #coverProfileCard>
        <div
          class="hidden md:flex items-center mx-auto mt-2 mb-4 shadow-lg bg-cover bg-no-repeat rounded-lg"
      ...
      <template #mainContent>
        ...
        <div
          class="mt-2 md:mt-0 mb-4 flex justify-center bg-white p-2 rounded-lg border-t-4 border-blue-500"
        >
          <button
            class="flex items-center justify-center max-w-fit px-4 py-2 my-1 font-medium text-center text-white rounded-md bg-green-400"
    ...
    <div
      v-if="props.donates.data[currentIndex].donor"
      :class="{ hidden: !showDonorProgress }"
      class="fixed top-0 left-0 z-20 flex items-center justify-center w-full h-full modal bg-gray-900/100 modal-overlay"
    >
      <div class="relative w-[40rem] h-[22rem] z-30 bg-white rounded-lg mx-auto overflow-hidden">
        <figure class="flex items-center p-2 mb-4 bg-gray-300 rounded-md">
          <div class="flex-shrink-0">
            <img
              class="w-40 h-40 rounded-full border-4 border-yellow-400"
          ...
          <div class="w-full ps-3">
            <div class="flex flex-col mb-1 text-[40px] text-gray-700 dark:text-gray-400">
              <span class="font-bold text-blue-500">{{
                props.donates.data[currentIndex].donor.name
              }}</span>
              <span class="font-semibold">{{
                props.donates.data[currentIndex].donor.personal_code
              }}</span>
            </div>
          </div>
        </figure>
        ...
        <div class="absolute top-[75%] left-[14rem] text-[60px] text-yellow-400/80">
          {{ donateRecieved }} แต้ม
        </div>
      </div>
    </div>
    ...
    <div
      v-show="!props.donates.data[currentIndex].donor"
      :class="{ hidden: !showDonorProgress }"
      class="fixed top-0 left-0 z-20 flex items-center justify-center w-full h-full modal bg-gray-900/100 modal-overlay"
    >
      <div class="relative w-[40rem] h-[39rem] z-30 bg-white rounded-lg mx-auto overflow-hidden">
        ...
        <ProgressBar mode="indeterminate" style="height: 16px"></ProgressBar>
        <div class="absolute top-[30%] left-[14rem] text-[60px] text-yellow-300/80">
          {{ donateRecieved }} แต้ม
        </div>
      </div>
    </div>
