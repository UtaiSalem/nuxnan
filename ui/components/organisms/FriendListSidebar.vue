<script setup lang="ts">
import { useFriends } from '~/composables/useFriends'
import { useChatStore } from '~/stores/chat'

const { friends } = useFriends()
const chatStore = useChatStore()
</script>

<template>
  <div class="fixed-sidebar right bg-white dark:bg-gray-800 transition-colors duration-300">
    <div class="chat-friendz dark:bg-gray-900">
      <ul class="chat-users">
        <li v-for="friend in friends" :key="friend.id">
          <div class="author-thmb">
            <img :src="friend.avatar" :alt="friend.name">
            <span class="status" :class="friend.status"></span>
          </div>
        </li>
      </ul>
      
      <!-- Chat Box -->
      <div class="chat-box" v-if="chatStore.activeChat">
        <div class="chat-head">
          <span class="status f-online"></span>
          <h6 class="dark:text-white">{{ chatStore.activeChat.name }}</h6>
          <div class="more">
            <div class="more-optns"><i class="ti-more-alt"></i>
              <ul>
                <li>block chat</li>
                <li>unblock chat</li>
                <li>conversation</li>
              </ul>
            </div>
            <span class="close-mesage" @click="chatStore.closeChat"><i class="ti-close"></i></span>
          </div>
        </div>
        <div class="chat-list">
          <ul>
            <li class="me">
              <div class="chat-thumb"><img src="https://placehold.co/50x50" alt=""></div>
              <div class="notification-event">
                <span class="chat-message-item">
                  HI, Jack i have faced a problem with your software. are you available now, when i install this i have to received an error.
                </span>
                <span class="notification-date"><time datetime="2004-07-24T18:18" class="entry-date updated">Today at 2:12pm</time></span>
              </div>
            </li>
            <li class="you">
              <div class="chat-thumb"><img src="https://placehold.co/50x50" alt=""></div>
              <div class="notification-event">
                <span class="chat-message-item">
                  Hi tina, Please let me know your purchase code, and show me the screnshot of error.
                </span>
                <span class="notification-date"><time datetime="2004-07-24T18:18" class="entry-date updated">Today at 2:14pm</time></span>
              </div>
            </li>
          </ul>
          <form class="text-box">
            <textarea placeholder="Post enter to post..."></textarea>
            <div class="add-smiles">
              <span title="add icon" class="em em-expressionless"></span>
              <div class="smiles-bunch">
                <i class="em em---1"></i>
                <i class="em em-smiley"></i>
                <i class="em em-anguished"></i>
                <i class="em em-laughing"></i>
                <i class="em em-angry"></i>
                <i class="em em-astonished"></i>
                <i class="em em-blush"></i>
                <i class="em em-disappointed"></i>
                <i class="em em-worried"></i>
                <i class="em em-kissing_heart"></i>
                <i class="em em-rage"></i>
                <i class="em em-stuck_out_tongue"></i>
              </div>
            </div>
            
            <button type="submit"></button>
          </form>
        </div>
      </div>
    </div>
  </div>
</template>
