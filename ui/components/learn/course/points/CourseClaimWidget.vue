...
const { items, pagination, meta, loading, error, fetchClaimable } = useCourseClaimable(props.courseId)

const dailyRemaining = computed(() => Math.max(0, 20 - meta.value.my_claims_today_all_tiers))
const totalPool = computed(() => meta.value.pool_total_remaining > 0 ? meta.value.pool_total_remaining : items.value.reduce((sum, item) => sum + item.remaining_points, 0))
