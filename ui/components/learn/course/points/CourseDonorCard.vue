...
const props = defineProps<{ donation: CourseDonationClaimable; courseId: number; capReached?: boolean }>()
const emit = defineEmits<{ claimed: [payload: unknown] }>()
const { claimFromDonation } = useCourseClaimable(props.courseId)
...
const splitPreview = computed(() => {
  ...
  const course = cost - claimer - suggester - platform
  return { cost, claimer, suggester, course, platform }
})
...
const mapErrorCodeToMessage = (code: string | number) => {
  switch (code) {
    case 'not_a_course_member':
      return 'ต้องเข้าร่วมรายวิชาก่อนจึงจะกดรับได้'
    case 'daily_cap_reached':
      return 'ครบเพดานกดรับ 20 ครั้ง/วัน แล้ว'
    case 'per_donation_cap_reached':
      return 'กดรับจากรายการนี้ครบ 5 ครั้งวันนี้แล้ว'
    case 'insufficient_pool':
      return 'แต้มในกองทุนคงเหลือไม่เพียงพอ'
    case 'donation_not_claimable':
      return 'รายการบริจาคนี้ไม่สามารถกดรับได้แล้ว'
    default:
      return 'เกิดข้อผิดพลาดในการรับแต้ม'
  }
}
...
const claim = async () => {
  ...
  try {
    ...
    const code = err?.code ?? err?.data?.code ?? 'claim_failed'
    errorMessage.value = mapErrorCodeToMessage(code)
...
<template>
  <article class="donor-card group relative overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-sm transition-all duration-300 hover:shadow-lg hover:border-indigo-200 dark:border-gray-700/60 dark:bg-vikinger-dark-200 dark:hover:border-indigo-800/60">
    ...
    <div class="min-w-0 p-4 sm:p-5">
      <!-- Public-tier donor presentation adapted for course claim logic -->
      <figure class="flex items-center gap-3 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 p-3 dark:from-blue-950/30 dark:to-indigo-950/30">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">ผู้สนับสนุนรายวิชา · {{ new Date(donation.created_at).toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: 'numeric' }) }}</p>
        </div>
        ...
        <div v-if="showSplit" class="overflow-hidden">
          <div class="mt-2 rounded-lg bg-gray-50 p-3 dark:bg-gray-800/50">
            ...
            <div class="flex h-3 rounded-full overflow-hidden mb-2.5">
              ...
              <div class="bg-violet-400 transition-all" :style="{ width: '7.41%' }" :title="`รายวิชา ${splitPreview.course}`" />
            ...
            <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px]">
              ...
              <div class="flex items-center gap-1.5">
                ...
                <span class="text-gray-600 dark:text-gray-300">รายวิชา</span>
                <span class="ml-auto font-bold text-violet-600 dark:text-violet-400">{{ splitPreview.course }} pp</span>
              </div>
              <div class="flex items-center gap-1.5">
      ...
      <div class="mt-2 flex items-center justify-between">
        <!-- Disabled reason or error message -->
        <p v-if="disabledReason || errorMessage" class="text-[11px] text-amber-600 dark:text-amber-400">
          <Icon icon="mdi:alert-circle" class="inline h-3 w-3 mr-0.5" />{{ errorMessage || disabledReason }}
        </p>
        <span v-else />
        ...
        <div class="w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-gray-800">
          <div class="relative overflow-hidden bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 p-6 text-center text-white">
            <div class="relative z-10">
              ...
              <p class="mt-1 text-sm text-white/80">ผู้ให้การสนับสนุนรายวิชา</p>
