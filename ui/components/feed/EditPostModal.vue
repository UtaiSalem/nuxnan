...
// Image preview URLs for new images
...
const loadPostData = () => {
  ...
  selectedPrivacy.value = post.privacy_settings ?? 3
...
const updatePost = async () => {
  if (!postText.value.trim() && allImages.value.length === 0) return
  ...
  try {
    const options = {
      ...
      delete_images: deletedImageIds.value
    }
    ...
    // Handle background - check if it was removed
    if (selectedBackground.value && allImages.value.length === 0) {
    ...
    } else if (originalPost?.background_color || originalPost?.background_gradient) {
...
const handleImageSelect = (event) => {
  ...
  const totalImages = allImages.value.length + files.length
  const remaining = 20 - allImages.value.length
  selectedImages.value = [...selectedImages.value, ...files.slice(0, remaining)]
...
const removeImage = (index) => {
  ...
  if (image.isNew) {
    ...
    const newIndex = selectedImages.value.findIndex(f => URL.createObjectURL(f) === image.url)
    if (newIndex !== -1) {
...
const selectBackground = (bg) => {
  if (selectedBackground.value?.id === bg.id) {
    ...
    // Clear images when selecting background
    selectedImages.value = []
    existingImages.value = []
    deletedImageIds.value = props.post?.postImages?.map(img => img.id) || []
  }
}
        ...
        <div class="w-full max-w-2xl mx-4 mb-10 modal-content">
          <div class="bg-white dark:bg-vikinger-dark-300 rounded-xl shadow-2xl">
            ...
            <div class="p-4 max-h-[70vh] overflow-y-auto">
              ...
              <div v-if="allImages.length > 0" class="mb-4">
                <div class="flex flex-wrap gap-2">
                  <div v-for="(image, index) in allImages" :key="image.id || image.url" class="relative group">
                    <img :src="image.url" class="w-32 h-32 object-cover rounded-lg border border-gray-200 dark:border-vikinger-dark-50/30" />
                    <div v-if="image.isNew" class="absolute top-1 left-1 px-1.5 py-0.5 bg-green-500 text-white text-xs rounded">ใหม่</div>
                  ...
                  <button v-if="allImages.length < 20" @click="triggerImageInput" class="w-32 h-32 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg flex items-center justify-center hover:border-vikinger-purple hover:bg-vikinger-purple/5 transition-all">
                    <Icon icon="mdi:plus" class="w-8 h-8 text-gray-400" />
                  </button>
                </div>
                <p class="text-xs text-gray-500 mt-1">{{ allImages.length }}/20 รูป</p>
              </div>

              <!-- Background Picker -->
              <div v-if="showBackgroundPicker && allImages.length === 0" class="mb-4 p-3 bg-gray-50 dark:bg-vikinger-dark-200 rounded-lg">
            ...
            <div class="p-4 border-t border-gray-200 dark:border-vikinger-dark-50/30">
              <button @click="updatePost" :disabled="isSubmitting || (!postText.trim() && allImages.length === 0)" class="w-full py-3 px-4 bg-gradient-vikinger text-white font-semibold rounded-lg hover:shadow-vikinger transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
