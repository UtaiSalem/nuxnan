...
const removeImage = async (image, index) => {
    try {
        ...
        let delResp = await axios.delete(`/topics/${props.topic.id}/images/${image.id}`);
        if (delResp.status === 200){
            props.topic.images.splice(index, 1);
        }
    } catch (error) {
        console.error('Error deleting image:', error);
        alert('ไม่สามารถลบรูปภาพได้ กรุณาลองใหม่อีกครั้ง');
    } finally {
      ...
      <div class="p-2">
        <form @submit.prevent="submitFormHandler" class="space-y-4">
            ...
            <div v-if="topic.images.length || tempImages.length" class="grid grid-cols-3 gap-2">
                <div v-for="(image, index) in topic.images" :key="image.id" class="relative">
                    <img :src="image.image_url" alt="Preview" class="rounded-md border w-full h-32 object-cover" />
                    <button @click.prevent="removeImage(image, index)" :disabled="isDeletingImage" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 hover:bg-red-600">
