    ...
    try {   
        ...
        if (memberResp.data.success) {
            ...
            Swal.fire({
                icon: 'warning',
                title: 'แจ้งเตือน',
                text: memberResp.data?.msg || memberResp.data?.message || 'ไม่สามารถขอเข้าร่วมกลุ่มได้',
                confirmButtonText: 'ตกลง'
            });
        }
    } catch (error) {
        ...
        const errorMessage = error.response?.data?.msg || error.response?.data?.message || error.message || 'เกิดข้อผิดพลาดในการขอเข้าร่วมกลุ่ม';
        Swal.fire({
            icon: status === 403 ? 'warning' : 'error',
            title: status === 403 ? 'แจ้งเตือน' : 'ข้อผิดพลาด',
            text: errorMessage,
            confirmButtonText: 'ตกลง'
        });
    }
    ...
    try {
        ...
        if (unmemberGroupResp.data.success) {
            ...
            Swal.fire({
                icon: 'warning',
                title: 'แจ้งเตือน',
                text: unmemberGroupResp.data?.msg || unmemberGroupResp.data?.message || 'ไม่สามารถออกจากกลุ่มได้',
                confirmButtonText: 'ตกลง'
            });
        }
    } catch (error) {
        ...
        const status = error.response?.status;
        const errorMessage = error.response?.data?.msg || error.response?.data?.message || error.message || 'เกิดข้อผิดพลาดในการออกจากกลุ่ม';
        Swal.fire({
            icon: status === 403 ? 'warning' : 'error',
            title: status === 403 ? 'แจ้งเตือน' : 'ข้อผิดพลาด',
            text: errorMessage,
