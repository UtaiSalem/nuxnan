...
ปิดชุด responsive ครบทั้ง 3 ความกว้าง: 375px (มือถือ) · 768px (แท็บเล็ต) · desktop ไม่แตะ

### วิธีตรวจ — ห้ามเอากฎมือถือมาใช้ซ้ำตรงๆ

ที่ 768px คลาส `sm:` และ `md:` **ทำงานแล้ว** แต่ `lg:`/`xl:`/`2xl:` **ยังไม่ทำงาน**
ต้องเขียนตัว resolve "คลาสที่มีผลจริงที่ 768px" ก่อน แล้วค่อยรันกฎ:

- จัดกลุ่ม utility ตาม property family (`grid-cols`, `p`, `px`, `w`, `@display`, `@flexdir`)
- ในแต่ละกลุ่ม breakpoint ที่ **active และใหญ่สุด** ชนะ (base < sm < md)
- ข้าม variant ที่ไม่เกี่ยวกับ layout (`hover:` `dark:` `group-hover:`)
- สคริปต์: `scan-tablet.mjs` รับ argument ความกว้าง (640/768/1024) ใช้ซ้ำได้

### ผลลัพธ์

| หมวด | พบ | ของจริง |
|---|---|---|
| element กว้างเกิน 768px | **0** | — |
| ตารางไม่มี h-scroll / หน้าเลื่อนแนวนอน | **0** | — |
| กริด 4+ คอลัมน์ที่ 768px | 107 | **4** |
| `hidden` จนถึง `lg:`/`xl:` | 25 | **0** |

### 4 จุดที่แก้ (เลื่อนจำนวนคอลัมน์จาก `md:` ไป `lg:`)

| ไฟล์ | เปลี่ยน | หลักฐานที่วัดได้ |
|---|---|---|
| `landing/TeamSection.vue` | `md:4` → `sm:2 lg:4` | "Marketing Manager" 189px / ช่อง 96px |
| `admin/gradebook/transcripts/index.vue` | `md:5` → `md:3 lg:5` | "เกียรตินิยม (≥3.5)" 106px / ช่อง 81px |
| `academy/member/MemberList.vue` | `md:6` → `md:3 lg:6` | "รอการอนุมัติ" 73px / ช่อง 66px |
| `admin/student-cards/requests/index.vue` | `md:4` → `sm:2 lg:4` | select "ปีการศึกษา 2569 (ปัจจุบัน)" 198px / ช่อง 159px |

หลังแก้พอดีครบ 4/4 · จอ `lg:` ขึ้นไปเหมือนเดิมทุกจุด

### กับดักเฉพาะแท็บเล็ต (บันทึกไว้ใช้ครั้งหน้า)

1. **`grid-cols-12` ไม่ใช่ 12 คอลัมน์จริง** — เป็นโครงที่ลูกใช้ `col-span-N`
   คำนวณ `768/12 = 41px` แล้วสรุปว่าแน่นคือ **ผิด** ต้องเช็คว่าลูกมี `col-span` ไหมก่อน (เจอ 4 จุด)
2. **`hidden ... lg:block` บน `<aside>` ไม่ใช่บั๊ก** ถ้ามีปุ่ม toggle คู่กัน — ตรวจแล้วมีครบ:
   `main.vue` ปุ่ม `lg:hidden` (ซ้าย) + `xl:hidden` (ขวา) · `course.vue:114` ปุ่ม `lg:hidden`
   `GradebookScoreTable` card view `xl:hidden` · `groups/[groupId]` card view `lg:hidden`
   ⇒ แท็บเล็ตเข้าถึงเนื้อหาได้ทุกอัน **ห้ามไปแก้**
3. **ป้ายพอดีไม่ได้แปลว่าช่องพอ** — `student-cards/requests` ป้าย 154/159px พอดี (false positive)
   แต่ `<select>` ข้างในมีตัวเลือกยาว 198px รวมขอบ+ลูกศร โดนตัด
   ⇒ กริดที่มี form control **ต้องวัดเนื้อหาใน control ไม่ใช่แค่ป้าย**

### Dead component ที่เจอระหว่างทาง (ยังไม่ลบ)

- `ui/components/FormSection.vue` — ไม่มีใครเรียกใช้ (`grid-cols-6` ในนั้นไม่กระทบใคร)
- `ui/components/student-card/CardSyncPreviewPanel.vue` — ไม่มีใครเรียกใช้ (แน่นที่ 768px แต่ข้ามไป)

### งานที่ค้าง (TODO)

- [x] ~~**`npm run build`**~~ — **ผู้ใช้รันเองแล้ว ผ่าน ไม่มี error** (2026-08-28)
      รันบน tree ที่ `b78bba6b` ⇒ **ครอบคลุมงาน responsive ครบทั้ง 3 ความกว้าง**
      (มือถือ 375px · padding ซ้อน · แท็บเล็ต 768px) — build ผ่านแล้วทั้งชุด ไม่มี TODO build ค้าง
- [~] **เปิดหน้าจริงบนมือถือ/แท็บเล็ต** — Claude ยืนยันด้วยตาได้แค่หน้า public
      (`/courses`, landing) ที่ 375px และ 768px = **0 element ล้นแนวนอน ทั้งสองความกว้าง**
      - [x] **สมุดคะแนน** — ผู้ใช้เปิดหน้าจริงบนมือถือแล้ว **ปกติดี** (2026-08-28)
            ⇒ เป็นหน้าที่การ์ดซ้อนหลายชั้น + มีตารางในกล่องเลื่อนแนวนอน จึงครอบคลุมทั้ง
              `overflow-x-auto` ที่ห่อตาราง และการลด padding การ์ด — **ยืนยันบนเครื่องจริงแล้ว**
      - [ ] ที่เหลือ: จัดการบุคลากร · แดชบอร์ดผู้ปกครอง · ใบ ปพ. (transcripts)
            (ใบ ปพ. สำคัญสุดในสามอันนี้ — เป็นจุดเดียวที่แก้ `md:grid-cols-5` ของงานแท็บเล็ต)
- [ ] ถ้าจะบีบ padding ต่ออีก: ทำการ์ดชั้นในให้ไม่มีพื้นหลัง/ขอบบนมือถือ (~91%)
      **เปลี่ยนดีไซน์ ไม่ใช่แค่ระยะห่าง** — ต้องถามผู้ใช้ก่อน

### Branch / Git State

- Branch: `main`
- Uncommitted: `ui/pages/Learn/Courses/[id]/index.vue` — งาน guardians ของอีก session
  (มีบรรทัดของงาน responsive ปนอยู่ 1 บรรทัด ดูรายการ padding ด้านล่าง)
- Push: **pushed**

---

## 2026-08-28 (จบงาน padding มือถือ) — เลิกซ้อน padding แนวนอน + ลดขนาดการ์ด: 57% → 91% ของจอ (push แล้ว)

### สถานะ: **ปิดครบตามที่ผู้ใช้เคาะ (แบบ C)** — `f63edf74` · `3260b6b0`

ต่อจากรายการ responsive ด้านล่าง ผู้ใช้สังเกตเองว่า "การ์ดซ้อน 3 ชั้น = margin ซ้ายขวา 6 เท่า
เหลือพื้นที่แสดงผลน้อยมาก" — วัดแล้วจริง

| commit | ขอบเขต | จุด | ไฟล์ |
|---|---|---|---|
| `f63edf74` | layout + container ชั้นนอกสุดของหน้า → `px-0` บนมือถือ | 155 | 157 |
| `3260b6b0` | padding การ์ด `p-6/p-8/p-12` → `p-4 sm:p-*` | 473 | 224 |

### ตัวเลขที่วัดได้จริงที่ 375px (อ้างอิงได้)

**โครงที่ซ้อนกันก่อนแก้:** layout `px-4` 32px → หน้า `p-6` 48px → การ์ด `p-6` 48px
→ การ์ดใน `p-4` 32px = **กิน 160px เหลือเนื้อหา 215px (57% ของจอ)**
สแกนทั้งแอป: **508 ไฟล์ซ้อน padding ≥2 ชั้น มัธยฐานกิน 96px**

| สภาพ | ก่อน | หลัง |
|---|---|---|
| หน้า `/courses` ของจริง (การ์ด) | — | การ์ด 375px = 100% ของจอ · ข้อความ 333px (89%) |
| การ์ดชั้นเดียว | 327px | 343px (+16) |
| การ์ดซ้อน 2 ชั้น | 279px | 311px (+32) |
| การ์ดซ้อน 3 ชั้น | 231px (62%) | 279px (74%) (+48) |

### ตัวเลือกที่ผู้ใช้เคาะ (แบบ C — เก็บไว้อ้างอิงถ้าจะปรับต่อ)

- **B** การ์ดเต็มขอบ เหลือ padding ชั้นในสุดชั้นเดียว → 343px (91%) แต่การ์ดซ้อนกันดูรวมเป็นก้อนเดียว
- **C ← เลือกอันนี้** การ์ดนอกเต็มขอบ การ์ดในยังมี padding → 327px (87%) แยกชั้นออก อ่านง่ายกว่า
- **D** แค่ลดลงไม่ตัดทิ้ง → 303px (81%) เสี่ยงต่ำสุดแต่ได้พื้นที่น้อยสุด

### กับดักที่เจอรอบนี้ (สำคัญ)

1. **ถอด `px-4` จาก layout แล้ว 50 หน้าตัวหนังสือชนขอบจอ** — หน้าพวกนั้นไม่เคยมี padding ของตัวเอง
   อาศัย layout ล้วน ต้องคืน `px-4 sm:px-0` ให้ 43 หน้า
   (อีก 5 หน้า root เป็น `<BaseCard>` = เป็นการ์ดอยู่แล้ว เต็มขอบถูก · 1 หน้าเป็น `<NuxtPage/>` ล้วน)
2. **`departments/index.vue` เป็น multi-root** (`<NuxtPage v-if>` + `<div v-else>`) สคริปต์คว้า
   `<NuxtPage>` เป็น root ต้องแก้มือ — ตรงกับกับดัก multi-root ที่บันทึกไว้ก่อนหน้า
3. **ตัวกรอง "container ของหน้า" ต้องรัดให้แน่น** — รอบแรกไปโดน `<NuxtLink p-2>` (ปุ่มย้อนกลับ)
   และ `<p p-4>` ต้องจำกัดเป็น `div`/`main`/`section` + ต้องมีลูกอย่างน้อย 2 ตัว
4. **ลด padding การ์ดต้องข้ามตัวที่มี breakpoint จูนแกนเดียวกันอยู่แล้ว** ไม่งั้นทับของที่ปรับมือไว้
5. เลือกเป้าหมาย "ผิวการ์ด" ด้วยเงื่อนไข **มีทั้ง `bg-*` และ `rounded-*`/`shadow`** — ใช้อย่างเดียวไม่พอ

### สิ่งที่ทำเกินคำสั่ง (แจ้งผู้ใช้แล้ว)

ผู้ใช้สั่ง "ชั้น component" แต่ทำการ์ดที่เขียนอินไลน์ในหน้าด้วย (component 150 จุด + หน้า 323 จุด)
เพราะเป็นเลเยอร์เดียวกัน ถ้าแก้แค่ฝั่งเดียวจะได้การ์ด `p-4` วางข้างการ์ด `p-6` บนจอเดียวกัน

### วิธีตรวจที่ใช้ (เหมือนรอบก่อน)

`สแกน → patch → SFC compile ทุกไฟล์ที่แตะ → diff purity (ทุกบรรทัด + ต้องตรงรูปแบบเดียว)
→ เปิดหน้าจริงในเบราว์เซอร์ที่ 375px วัด element ล้น + ความกว้างเนื้อหา`

- `f63edf74`: compile 158/158 · diff บริสุทธิ์ 159/159 บรรทัด
- `3260b6b0`: compile 225/225 · diff บริสุทธิ์ 473/473 บรรทัด
- ทั้งสอง: หน้า `/courses` ที่ 375px **ไม่มี element ล้นแนวนอน หน้าไม่เลื่อนข้าง**

### งานที่ค้าง (TODO)

- [x] ~~**`npm run build`**~~ — **ผู้ใช้รันเองแล้ว ผ่าน ไม่มี error** (2026-08-28)
      รันบน working tree ที่ `31994f15` ⇒ **ครอบคลุมงาน responsive ทั้ง 7 commit สะสม**
      (`0d14170f` `2014a134` `7ecb0a16` `f63edf74` `3260b6b0`) ไม่ใช่แค่ 2 commit นี้
- [ ] **เปิดหน้าจริงบนมือถือ** — ยืนยันได้แค่หน้า public (`/courses`, `/schools`) ซึ่งบังเอิญ
      ไม่ได้ใช้ `p-6` เลย ตัวเลขจึงไม่ขยับ ตัวเลขการ์ดซ้อนในตารางมาจากวัด markup ในเบราว์เซอร์
      - [ ] หน้าที่ควรเปิดก่อน: สมุดคะแนน · จัดการบุคลากร · แดชบอร์ดผู้ปกครอง (การ์ดซ้อนหลายชั้น)
- [ ] ถ้าจะบีบต่ออีก: ทำการ์ดชั้นในให้ไม่มีพื้นหลัง/ขอบบนมือถือ (เหลือ padding ชั้นเดียวจริง → ~91%)
      **แต่นั่นเปลี่ยนดีไซน์ ไม่ใช่แค่ระยะห่าง** — ต้องถามผู้ใช้ก่อน

### Branch / Git State

- Branch: `main`
- Uncommitted: `ui/pages/Learn/Courses/[id]/index.vue` — งาน guardians ของอีก session
  **มีการแก้ของงานนี้ปนอยู่ 1 บรรทัด** (`px-0 py-6 sm:px-6` จากรอบ layout) เจ้าของงานจะได้ไปด้วยตอน commit
- Push: **pushed**
...
งานคนละชุดกับ touch target 44px (รายการถัดไปด้านล่าง) — รอบนี้คือ **layout/responsive**

| commit | ขอบเขต | จุด | ไฟล์ |
|---|---|---|---|
| `0d14170f` | ตารางถูก `overflow-hidden` ตัดทิ้ง | 21 | 19 |
| `2014a134` | กริดบีบข้อความไทย 5 · แถวฟอร์มบีบไม่ได้ 3 · กล่องล้นจอ 2 | 10 | 10 |
| `7ecb0a16` | ข้อความ error ถูกซ่อนบนมือถือ | 1 | 1 |

สแกน `components/` + `pages/` + `layouts/` (760 ไฟล์) — **HIGH เหลือ 0 ที่ actionable**

### วิธีที่ใช้ (ทำซ้ำได้ — สำคัญกว่าตัวผลลัพธ์)

สแกนเนอร์ static **เชื่อไม่ได้** — 579 finding แรก เหลือของจริงแค่ 30 (false positive 95%)
ต้อง **วัดจริงในเบราว์เซอร์ที่ 375px** ก่อนแก้ทุกครั้ง

1. parser ต้องไต่ ancestor จริง (tag-stack) ไม่ใช่ regex ดูรอบๆ — ไม่งั้น `min-w-[880px]`
   ที่อยู่ใน `overflow-x-auto` (ซึ่ง**ถูกอยู่แล้ว**) จะถูกรายงานว่าผิด
2. วัดกริด: ดึง label ที่เป็น literal ออกจาก template → เอาไปวัดความกว้างจริงแบบ `white-space:nowrap`
   เทียบกับความกว้างช่องที่คำนวณจาก cols/gap/padding → **321 finding เหลือของจริง 5**
3. ทริค: ก๊อป JSON ไปวางที่ `ui/public/` ให้ dev server เสิร์ฟ แล้วให้หน้าเว็บ `fetch()` เอง
   (ดีกว่าแปะ payload 20KB เข้า tool call) **ลบไฟล์ทิ้งหลังวัดเสร็จ**
4. ตรวจทุกครั้ง: `git diff -w` ดูบรรทัดที่เปลี่ยนจริง → เปิด/ปิด tag ต้องเท่ากัน → SFC compile ทุกไฟล์ที่แตะ

### กับดักที่เจอ (อย่าไปแก้ — ของถูกอยู่แล้ว)

1. **`<table v-else>` ห่อ div ไม่ได้ตรงๆ** — `v-else` จะหลุดคู่ `v-if` แล้วพังทันที ต้องยก directive ขึ้น wrapper (เจอ 5 จุด)
2. `min-w-[Npx]` / `style="min-width"` บนตาราง = **ถูก** ถ้ามี ancestor เป็น `overflow-x-auto`
3. `flex-row sm:flex-col` = **ตั้งใจ** (มือถือไอคอน+ป้ายเรียงแถว เดสก์ท็อปซ้อน) ไม่ใช่ desktop-first
4. `w-96`/`w-[400px]` บน `absolute` + `blur-*` = วงกลมเบลอตกแต่งใน `overflow-hidden` ไม่มีทางล้น
5. `<table class="hidden md:table">` คู่กับ `block md:hidden` card view = **pattern ที่ถูก**
   (`GradebookScoreTable` ใช้ `xl:hidden`, `MemberList` ใช้ `block md:hidden`)
6. แถวฟอร์ม**ถูกแล้ว**ถ้าตัวหนึ่ง `flex-shrink-0` อีกตัว `min-w-0 flex-1` — ไม่ต้องมี `min-w-0` ทั้งคู่
7. ปุ่มไอคอน 44px 3 ปุ่มในแถว flex **ไม่ใช่ปัญหาความกว้าง** — นับเฉพาะ `input`/`select`/`textarea`
8. **เปลี่ยน `grid-cols-3` เป็น `grid-cols-1` พังลูกที่มี `col-span-2`** (สร้างคอลัมน์ผีแล้วล้น)
   ต้องแก้ span เป็น `sm:col-span-2` คู่กันเสมอ

### ตัวเลขที่วัดได้จริง (อ้างอิงได้)

- ตาราง staff 5 คอลัมน์ = **749px** บนจอ 375px ⇒ ซ่อน 376px (คอลัมน์ สถานะ + จัดการ)
  `overflow-hidden` ทำให้**นิ้วเลื่อนไปดูไม่ได้เลย** ต่างจาก `overflow-x-auto`
- `parent.vue` ที่ 4 คอลัมน์: ช่องป้ายกว้าง **41px** แต่ "ขาดเรียน" ต้องการ **52px** ⇒ แตกกลางคำ
  (ไทยไม่มีช่องว่าง) แก้เป็น `grid-cols-2` ได้ **131px** บรรทัดเดียว
- อีก 4 จุด: `ยอดโอนสุทธิ (รับจริง)` 110/86 · `โรงเรียน (Academy)` 127/97 ·
  `ชื่อวิชา (ภาษาไทย) *` 117/103 · `แจกให้นักเรียนแล้ว` 93/85 — หลังแก้พอดีครบ 5/5

### ที่วัดแล้ว "ไม่พัง" — ปิดเคสแล้ว ไม่ต้องกลับมาดูอีก

- **กริด 318 จุดที่เหลือ**: วัดครบ 225 กริดที่มีข้อความจริง แน่นแค่ 11 และ 6 ในนั้นเป็นขยะจากตัวดึงข้อความ
  ที่เหลือเป็น skeleton / ตัวเลือกสี / ธัมบ์เนล / กระดานเกม (XO, CrossMath) — ตั้งใจให้เป็นหลายคอลัมน์
- **`hidden` 200 จุด**: เป็นป้ายข้างไอคอน (92) หรือมีคู่มือถืออยู่แล้ว (37) หรือ decoration
  points/wallet ที่ซ่อนใน header มีลิงก์ในเมนูมือถือ (`main.vue:1233`) — เข้าถึงได้
- **`@media (max-width:)` 4 จุด**: หนี้ convention ไม่ใช่บั๊ก `.coupons-grid` ใช้
  `auto-fill minmax(300px,1fr)` ซึ่ง responsive ในตัวอยู่แล้ว · `.stats-bar` มี `flex-wrap` แล้ว
- `ui/components/FormSection.vue` — **ไม่มีใครเรียกใช้เลย (dead component)** `grid-cols-6` ในนั้นจึงไม่กระทบใคร

### งานที่ค้าง (TODO)

- [ ] **เปิดหน้าจริงบนมือถือยืนยัน** — เข้าหน้าที่ต้อง login ไม่ได้ (ห้ามกรอกรหัสผ่าน)
      ยืนยันได้แค่หน้า landing ที่ 375px = **0 element ล้นแนวนอน**
      ที่เหลือยืนยันด้วยการวัด markup จริงในเบราว์เซอร์ + SFC compile ผ่าน 26/26 ไฟล์ที่แตะ
      - [ ] หน้าที่ควรเปิดจริงก่อน: สมุดคะแนน · จัดการบุคลากร · นำเข้านักเรียน · กระเป๋าเงิน admin
- [x] ~~`npm run build`~~ — **ผู้ใช้รันเองแล้ว ผ่าน ไม่มี error** (2026-08-28) ครอบคลุมชุดนี้ด้วย
      (รันทีเดียวบน tree ที่ `31994f15` — ดูรายการ padding ด้านบน)
- [ ] ข้อสมมติของการวัดกริด: page padding = 32px หน้าไหนมี card ซ้อนอีกชั้นจะแคบกว่านี้
      ⇒ ตัวเลขเป็น **ขอบล่าง** อาจมีจุดแน่นเพิ่มที่ประเมินว่าผ่าน
- [ ] `isomorphic-dompurify` ถ้าต้องการ sanitize ระดับเดียวกันฝั่งเซิร์ฟเวอร์ (ลาก jsdom มาด้วย) — ค้างจากรอบก่อน

### Branch / Git State

- Branch: `main`
- Uncommitted: `ui/pages/Learn/Courses/[id]/index.vue` (งาน guardians ไม่ใช่ของชุด responsive — ไม่ได้แตะ)
- หมายเหตุ: `1d816fa5` (guardians G-S12e) ถูก commit ระหว่าง session นี้จากที่อื่น ไม่ใช่งานชุดนี้
- Push: **pushed**
