...
### งานที่ค้าง (TODO ต่อ — เหลือ 3)

- [ ] **เปิดหน้าจริงบนมือถือยืนยัน** — งานทั้งหมดตรวจผ่าน harness (เข้าหน้าที่ต้อง login ไม่ได้)
      โดยเฉพาะ **แถบเครื่องมือ RichTextEditor** ที่เปลี่ยนเป็นเลื่อนแนวนอน ควรลองใช้จริงบนมือถือ
- [ ] `isomorphic-dompurify` ถ้าต้องการ sanitize ระดับเดียวกันฝั่งเซิร์ฟเวอร์ (ลาก jsdom มาด้วย)
- [ ] `npm run build` ผู้ใช้รันเอง — ยังไม่ได้รันตลอดงานชุดนี้ (ตรวจแค่ SFC compile + `tsc --noEmit`)
