...
### ข้อมูลทดสอบ — **ยังคาไว้ในฐาน dev ตั้งใจไม่ลบ** (ต้องใช้ตรวจซ้ำหลังแก้)

election **18** (published) / **19** (nomination) / **20** (draft) · พรรค 7–10 · หน่วย 11, 12
ผู้มีสิทธิ์ 2,193 แถว · บัตร 14 ใบ · **และ `academy_groups` id 40 (`student_council`) จากการกดปุ่มจริง**
คำสั่งล้างอยู่ใน §15.6 + §16.5 ของสเปก
...
### 🟡 ที่เพิ่งเห็นเพราะแท็บกลับมาเรนเดอร์ (ยังไม่แก้ — ดู §16.4)

role/สถานะพรรคเป็นอังกฤษดิบ · ปุ่มอนุมัติ/ปฏิเสธยังโชว์กับพรรคที่อนุมัติแล้ว · แท็บผลโชว์คีย์ดิบ
`voted/total/percentage` · แท็บหน่วยโชว์ `ออกบัตร 0` ทั้งที่ `ใช้สิทธิ์ 14` (อ่านแล้วเข้าใจผิดง่ายในวันจริง)
· แท็บบันทึกช่องผู้ทำรายการเป็น `-`

### งานที่ค้างต่อ

- [ ] เก็บ 6 ข้อใน §16.4 (ป้ายไทย + ปุ่มที่ไม่ควรโชว์ + `issued_count` ที่อ่านแล้วเข้าใจผิด) แล้วเปิดจอตรวจซ้ำทั้ง 6 แท็บ + apply.vue + ปุ่มตั้งสภานักเรียน
- [ ] ตรวจ 16 จุดที่เหลือจากการกวาด (rollover / revenue / BaseCard / CourseCard)
- [ ] ล้างข้อมูลทดสอบ 18/19/20 หลังตรวจซ้ำเสร็จ
- [ ] **ซ้อมการเลือกตั้งกับนักเรียน 1 ห้องตาม §9 — ยังไม่ได้ทำ** (งานที่ไม่ใช่โค้ด)
