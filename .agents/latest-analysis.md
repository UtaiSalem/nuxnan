...
### 2026-06-06 - Codex personal code card runtime fix
- Scope: `ui/layouts/main.vue`, `ui/components/user/PersonalCodeCard.vue`, `ui/components/molecules/RegisterForm.vue`, `ui/components/organisms/AuthCard.vue`, `ui/stores/auth.ts`.
- Findings: the personal-code UI change left a broken duplicated Games menu fragment in `main.vue` and added unnecessary `skipHydrate(isLoginTransitioning)` in the auth store, matching the Nuxt/Pinia SSR payload error path (`obj.hasOwnProperty is not a function`). Focused typecheck also reported `$fetch<T>` generic usage in touched auth/register files.
- Change: removed the broken template fragment, restored `isLoginTransitioning` as normal store state, changed touched `$fetch<T>` calls to post-call casts, and kept the new personal-code card plus `/auth?tab=register&ref=...` flow.
- Verification: focused `vue-tsc` output filter for `layouts/main.vue`, `PersonalCodeCard.vue`, `stores/auth.ts`, `AuthCard.vue`, and `RegisterForm.vue` returned no matching errors; `Invoke-WebRequest http://localhost:3000/play/newsfeed` returned 200; `Invoke-WebRequest http://localhost:3000/auth?tab=register&ref=11111111` returned 200; `git diff --check` passed for touched UI files.

### 2026-06-06 - Codex admin user update 422 fix
- Scope: `api/nuxnanravel/app/Http/Controllers/Api/AdminController.php`, `ui/pages/nuxnan-admin/users/[id]/edit.vue`.
- Findings: `PUT /api/admin/users/{id}` could return 422 because the admin edit UI displayed a required username but did not initialize or submit it, while backend update validation still treated display `name` as unique. A second email-edit 422 was likely caused by username fallback being submitted during email-only edits, making unrelated username validation block the request.
- Change: admin edit now initializes `username`, only submits it when it was intentionally changed, trims editable strings, and normalizes validation error arrays for display. Backend update trims string inputs, validates uniqueness on `users.username` instead of display `name`, skips unchanged username validation, allows duplicate display names, and normalizes `roles` before `exists:roles,name`.
- Verification: `php -l app\Http\Controllers\Api\AdminController.php` passed; `git diff --check -- api/nuxnanravel/app/Http/Controllers/Api/AdminController.php ui/pages/nuxnan-admin/users/[id]/edit.vue` passed; focused `vue-tsc` output filter for `pages/nuxnan-admin/users/[id]/edit.vue` returned no matching errors. `php artisan route:list --path=admin/users` is blocked by an unrelated parse error in `CourseCompletionController.php`.

### 2026-06-06 - Codex admin users edit modal
- Scope claimed: `ui/pages/nuxnan-admin/users/index.vue`.
- Findings: the admin users table currently uses a pencil `NuxtLink` to `/nuxnan-admin/users/{id}/edit`; the existing edit page already has the update contract and recent username validation fixes. The list page already uses local Teleport modals for verify/delete, so an inline edit modal matches local UX.
- Plan: add edit modal state and handlers in the list page, fetch the selected user detail on modal open, submit updates to the existing admin user update endpoint, replace the pencil link with a modal button, refresh the list after save, and run focused frontend checks.
- Change: added an inline edit-user modal to `users/index.vue`, including detail loading, client validation, normalized Laravel validation errors, password-optional update, role/status/admin flags, and table refresh after successful save. The pencil action now opens this modal instead of routing to the edit page.
- Verification: `git diff --check -- ui/pages/nuxnan-admin/users/index.vue .agents/latest-analysis.md` passed; focused `vue-tsc` output filter for `pages/nuxnan-admin/users/index.vue` returned no matching errors; `Invoke-WebRequest http://localhost:3000/nuxnan-admin/users` returned 200. In-app browser click verification was not available because no browser control tool was exposed in this turn.
