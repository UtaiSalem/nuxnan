...
| 10 | ห้องเรียน | `admin/classrooms/` | `groups.view` / `groups.manage` | [10-classrooms.md](10-classrooms.md) | 🟡 **audit + CL-S1/S2/S3/S6 done 2026-09-15** — ปิดครบทุก step ที่ไม่ติด Q (CL-S1 wherePivot status=2 `03704c4e` · CL-S2 prod-guard command `c8176236` · CL-S3 FE roster 405 `d358cce0` · CL-S6 mobile static-clear) · **เหลือ CL-S4 รอ Q1–Q3** (ยกด่าน `canManage`→`Academy::userCan('groups.manage')` G18) + CL-S5 เทสต์ · CL-S7 legacy-column deferred · หลักฐาน: pint+ClassroomManagementTest 19/19 · ยังไม่ตรวจจอจริง CL-S3 |
