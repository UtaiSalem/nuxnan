...
| 10 | ห้องเรียน | `admin/classrooms/` | `groups.view` / `groups.manage` | [10-classrooms.md](10-classrooms.md) | 🔴 **audit เสร็จ 2026-09-15** — พบ gap 7 ข้อ · ต่างจาก #8: route มีด่าน `canManage()` ในโค้ดจริง (ไม่ใช่ประตูเปิด) แต่ **bypass ระบบ permission-key/ฝ่าย** (G1) + ไม่กรอง status สมาชิก (G2) · console command ล้างห้องทั้งโรงเรียนยังอยู่ (G4) · แตกเป็น CL-S1–S7 · **ทำได้ทันที: CL-S1/S2/S3/S6** · CL-S4 รอ Q1–Q3 · memory [[project-classroom-source-of-truth]] ล้าสมัยเรื่อง deleteClassroom (ปิดแล้ว) |
