...
| **S-S6b** | หน้าจอ: ตารางแข่ง/สายการแข่ง · กรอกผลรายแมตช์ · จอยืนยันอันดับ · สรุปเหรียญละเอียด | S-S5c | ✅ 2026-08-20 — `useSportsMatches` + `SportsMatchBoard` + `SportsPlacingPanel` + 2 แท็บในหน้า `admin/sports-scores` · ตรวจในเบราว์เซอร์จริงที่ 375/768/1280 แล้ว ดู §12.9 |
| **S-S7** | อัลบั้มภาพผูกกับงาน (ต้องเพิ่ม owner ให้ `albums` หรือทำตารางใหม่ — ดู §1.3) | S-S6b | ✅ **2026-08-20** — ตารางใหม่ `sports_albums`/`sports_photos` + 9 route + ย่อรูปจริง (13 เทสต์) + หน้าจอแท็บ "อัลบั้มภาพ" ดู §12.10 · **ยังต้องรัน migration เอง** |
...
## 12.9 S-S6b — หน้าจอตารางแข่ง + ยืนยันอันดับ (2026-08-20)

ไฟล์ใหม่ 3 ไฟล์ + ต่อ 2 แท็บเข้าหน้าเดิม:

| ไฟล์ | หน้าที่ |
|---|---|
| `ui/composables/useSportsMatches.ts` | ห่อ 8 endpoint ของ S-S5a/b/c + helper `groupByRound` `matchStatusText` `participantStatusText` `formatTimeMs` |
| `ui/components/academy/sports/SportsMatchBoard.vue` | เลือกรายการแข่ง · สร้างตารางอัตโนมัติ (3 รูปแบบ) · ดูสายเป็นรอบ ๆ · กรอกผลรายคู่แบบ inline |
| `ui/components/academy/sports/SportsPlacingPanel.vue` | ดึงอันดับที่ระบบเสนอ → แก้ได้ → ยืนยัน → คะแนนเข้า + สรุปเหรียญของรายการนั้น |
| `ui/pages/academies/[name]/admin/sports-scores/index.vue` | +2 แท็บ `ตารางแข่ง` (สิทธิ์ `sports.view`) และ `ยืนยันอันดับ` (เฉพาะ `canManage`) |

### สิ่งที่หน้าจอต้องบอกผู้ใช้ (ล็อกไว้ ห้ามถอดออกตอน refactor)

1. ใต้ฟอร์มกรอกผลรายคู่: **"การบันทึกผลรายคู่ยังไม่ใช่การให้คะแนนคณะสี"** — `PUT /matches/{id}/result` ไม่แตะ `sports_score_entries` เลย ถ้าไม่บอกครูจะรอคะแนนที่ไม่มีวันมา
2. ก่อนสร้างตารางแข่ง: เตือนว่าจะลบคู่ที่ยังไม่แข่งทิ้ง + ถ้ามีแมตช์ที่แข่งจบแล้วจะโดน 422 (แสดงข้อความจริงจาก API)
3. ก่อนยืนยันอันดับ: เตือนว่าจะ **void คะแนนอันดับเดิมของรายการนี้ทั้งหมด รวมที่ครูลงเอง** แล้วลงชุดใหม่
4. อันดับซ้ำ = **เตือนอย่างเดียว ห้ามบล็อก** (อันดับร่วมถูกต้องตาม §10.2) · ที่บล็อกคือคณะซ้ำหรืออันดับว่าง/ต่ำกว่า 1
5. คะแนนในตารางร่างเป็น **ตัวอย่าง** เท่านั้น ค่าจริงมาจาก API

### ✅ ตรวจในเบราว์เซอร์จริงแล้ว (หน้า preview ชั่วคราวพร้อมข้อมูลจำลอง ลบทิ้งแล้ว)

- **375px**: `document.documentElement.scrollWidth === 375` · องค์ประกอบที่ล้นขอบ = **0 ชิ้น** · ปุ่ม/ช่องกรอก/label ของ checkbox สูง ≥ 44px ทุกตัว
- **768px / 1280px**: ไม่มีองค์ประกอบล้นขอบเช่นกัน
- เลือกรายการแข่งอัตโนมัติได้ · `format` ของรายการถูกเติมลงฟอร์ม (knockout → checkbox "สร้างคู่ชิงอันดับที่ 3" โผล่ตามเงื่อนไข)
- เพิ่มคณะในตารางร่าง → เหตุผลเปลี่ยนเป็น "ครูกำหนดเอง" · ตั้งอันดับ 1 ซ้ำสองคณะ → ขึ้นคำเตือนอันดับร่วม **แต่ปุ่มยืนยันยังกดได้** และคะแนนตัวอย่างออกมา 9 เท่ากันทั้งคู่ (ตรงตาราง `{1:9}`)

### 🔴 บั๊กที่เจอตอนเปิดจริง (ถ้าไม่เปิดเบราว์เซอร์จะไม่มีทางเห็น)

`watch(..., { immediate: true })` ที่เฝ้า `props.disciplines` ถูกประกาศ **ก่อน** `watch(selectedDisciplineId, ...)`
⇒ callback ของ immediate ทำงานตอน setup ทันที ตั้งค่า `selectedDisciplineId` ตั้งแต่ยังไม่มีใครเฝ้า
⇒ **แมตช์ไม่ถูกโหลด และฟอร์มไม่ถูกตั้งค่าเลย** (จอขึ้น "ยังไม่มีแมตช์" ตลอด และ format ค้างที่ `round_robin`)
แก้โดยย้ายลำดับการประกาศ watch และเขียนคำเตือนไว้ในไฟล์แล้ว — **ระวังรูปแบบนี้ซ้ำในหน้าอื่น**

### ที่ยังไม่ได้ทำ/ยังไม่ได้ทดสอบ
- **ยังไม่ได้ทดสอบกับ API จริง** (ต้องล็อกอินเป็นแอดมิน academy) — ทุกอย่างข้างบนทดสอบด้วยข้อมูลจำลอง
- ยังไม่มีการวาดสายแข่งแบบ bracket เป็นเส้นเชื่อม (ตอนนี้แสดงเป็นการ์ดจัดกลุ่มตามรอบ ซึ่งอ่านง่ายกว่าบนมือถือ)
- สรุปเหรียญเป็นระดับ "รายการแข่ง" เท่านั้น · ภาพรวมทั้งงานยังอยู่ที่ `SportsStandingsBoard` เหมือนเดิม
- ต่อไปคือ **S-S7** (อัลบั้มภาพผูกกับงาน) ซึ่งไม่มีอะไรบล็อกแล้ว

---

## 12.10 S-S7 (backend) — อัลบั้มภาพงานกีฬาสี (2026-08-20)

### ข้อตัดสินที่ผู้ใช้เลือกเอง
1. **สร้างตารางใหม่ `sports_albums` + `sports_photos`** (ไม่ต่อ owner ให้ตาราง `albums` เดิม) — ความเสี่ยงต่อระบบอัลบั้มส่วนตัวของผู้ใช้ = 0 · แลกกับที่ไม่ได้ไลก์/คอมเมนต์มาฟรี
2. **ย่อรูปจริงด้วย intervention/image v3** ในงานนี้เลย

### 🔴 §1.2 ของเอกสารนี้เคยเขียนผิด — แก้ความเข้าใจ
เดิมเขียนว่า `PostMediaService` = "อัปโหลดหลายใบ + ย่อ 2048/400 + quality 85 แม่แบบสำหรับอัลบั้มกีฬา"
**ของจริงโค้ดย่อรูปกับ thumbnail ในนั้นถูก comment ทิ้งทั้งหมด** และทั้งเรพไม่มีการย่อรูปที่ทำงานจริงเลยสักที่
(`AcademyController` ก็ comment เหมือนกัน) สาเหตุคือ intervention อัปเป็น v3 ซึ่งเลิกใช้ `Image::make()` แบบ v2
⇒ **S-S7 คือที่แรกในเรพที่ย่อรูปได้จริง** · API ที่ใช้ได้ (ทดสอบบนเครื่องนี้แล้ว GD เปิด Imagick ไม่มี):
`new ImageManager(new Gd\Driver())` → `read()` → `scaleDown(2048,2048)` → `cover(400,400)` → `toJpeg(85)`

### สิ่งที่ลงไปจริง
- migration `2026_08_20_000001_create_sports_album_tables.php` — 2 ตาราง key ที่ `edition_id` · ผูก `discipline_id`/`house_group_id` ได้ (nullable) · FK ปกอัลบั้ม (`sa_cover_photo_fk`) ผูกหลังสร้างตารางที่สอง แบบเดียวกับ `next_match_id`
- `SportsPhotoService` — ย่อลง 2048 (JPEG 85) + thumbnail ครอป 400×400 (JPEG 80) · เก็บที่ `images/sports/{edition_id}/` และ `/thumbs/` บน disk `public` · รูปแรกกลายเป็นปกอัตโนมัติ · ลบรูปปกแล้วปกเลื่อนไปใบถัดไป
- `SportsAlbumController` + 9 route (ดู = `sports.view` · แก้ = `sports.manage`) · รูปถูก scope ด้วย `edition_id` ทุกเส้น
- โมเดลมี accessor `url` / `thumbnail_url` ใน `$appends` ⇒ หน้าจอเอาไปใช้ได้เลย

### 🔴 บั๊กหน่วยความจำที่เจอตอนรันเทสต์ (สเปคผมเองสั่งผิด)
สเปครอบแรกสั่งให้ **ถอดรหัสไฟล์ต้นฉบับ 2 ครั้ง** (ครั้งหนึ่งทำรูปเต็ม อีกครั้งทำ thumbnail)
⇒ รันทั้งคลาสแล้ว `Allowed memory size of 134217728 bytes exhausted` ที่ `Gd\Cloner.php`
แก้เป็น **ถอดรหัสครั้งเดียว แล้วทำ thumbnail ต่อจากรูปที่ย่อแล้ว** + `unset($image)` ก่อนวนไฟล์ถัดไป
⇒ ผลพลอยได้ในการใช้งานจริง: อัปรูป 12MP หลายใบต่อคำขอเดียวไม่ระเบิด memory_limit ของเว็บ

**และตั้ง `<ini name="memory_limit" value="512M"/>` ใน `phpunit.xml`** — งานภาพต้องการมากกว่า 128M ของ php.ini

### ⚠️ ต้องรัน migration เอง — ตอนนี้ค้าง 2 ตัว
`php artisan migrate:status` บอกว่า **ยัง Pending 2 ตัว**:
- `2026_08_17_000002_create_sports_match_tables` (ของ S-S5a — **ไม่เคยถูกรันบน MySQL เลย**)
- `2026_08_20_000001_create_sports_album_tables` (ของรอบนี้)
⇒ ฟีเจอร์แมตช์/อันดับ/อัลบั้มทั้งหมด **ยังไม่เคยแตะ DB จริง** เทสต์ที่ผ่านทั้งหมดรันบน sqlite in-memory
ตรวจ SQL ล่วงหน้าด้วย `php artisan migrate --pretend` แล้ว — สร้างตารางใหม่ล้วน ไม่มี ALTER ตารางเดิม ไม่มีการแตะข้อมูล

### ที่ยังไม่ได้ทำ
- **หน้าจออัลบั้ม** (อัปโหลด/แกลเลอรี/ตั้งปก) ยังไม่ได้ทำ — เป็นงานถัดไปของ S-S7
- ไม่มีการจำกัดโควตาพื้นที่ต่อ academy · ไม่มีการลบไฟล์กำพร้าเมื่อ edition ถูกลบ (FK cascade ลบแถว แต่ไฟล์บน disk ยังอยู่)

### 12.10.1 หน้าจออัลบั้ม (2026-08-20)

| ไฟล์ | หน้าที่ |
|---|---|
| `ui/composables/useSportsAlbums.ts` (ใหม่) | ห่อ 9 endpoint · อัปโหลดด้วย `FormData` (`photos[]` + `captions[i]`) ผ่าน `useApi` ซึ่งรองรับ FormData อยู่แล้ว |
| `ui/components/academy/sports/SportsAlbumBoard.vue` (ใหม่) | 2 โหมดในไฟล์เดียว: รายการอัลบั้ม ↔ แกลเลอรีในอัลบั้ม · อัปโหลดหลายใบ · ตั้งปก/ลบรูป · lightbox เขียนเอง (ไม่ลง library) |
| `ui/pages/academies/[name]/admin/sports-scores/index.vue` | +แท็บที่ 7 "อัลบั้มภาพ" (ไม่มี `v-if` — `sports.view` ดูได้) · **+18 / −1** |

**กติกาที่ล็อกไว้ในหน้าจอนี้**
- ปุ่ม "ตั้งเป็นปก"/"ลบ" บนการ์ดรูป **ห้ามใช้ `group-hover` เป็นทางเดียว** — มือถือไม่มี hover จะกดไม่ได้เลย (ตอนนี้แสดงถาวรบนแถบ gradient)
- ห้ามประกอบ URL รูปเอง ใช้ `url` / `thumbnail_url` จาก accessor ของ API · กริดใช้ `thumbnail_url` เท่านั้น
- ห้ามย่อรูปฝั่งหน้าจอ (API ย่อให้แล้ว) · ตรวจ 8MB / 20 ไฟล์ / jpeg-png-webp ฝั่งหน้าจอด้วยเพื่อไม่ให้ผู้ใช้รออัปแล้วเด้ง 422
- lightbox ต้องคืน `document.body.style.overflow` ทั้งตอนปิด **และตอน `onUnmounted`**

**✅ ตรวจในเบราว์เซอร์จริงแล้ว** (หน้า preview ชั่วคราว ลบทิ้งแล้ว): 375px → `scrollWidth === 375` · องค์ประกอบล้นขอบ 0 ชิ้น · ไม่มีปุ่ม/ช่องกรอกต่ำกว่า 44px · เปิดฟอร์มสร้างอัลบั้มแล้วยังไม่ล้น · 1280px ล้น 0 ชิ้น
**ยังไม่ได้ตรวจของจริง**: แกลเลอรี/lightbox/อัปโหลด ต้องมีข้อมูลจาก API (ต้องรัน migration + ล็อกอินเป็นแอดมินก่อน) — ตรวจได้แค่ระดับคอมไพล์และอ่านโค้ด

**บั๊กที่แก้เอง 2 จุด**: การ์ดรูปเป็น `<button>` ที่มี `<button>` ซ้อนข้างใน (HTML ไม่ถูกต้อง event ไม่แน่นอน) → เปลี่ยนเป็น `div role="button" tabindex="0"` + รองรับ Enter/Space · label ของ checkbox "ให้คนทั่วไปดูได้" สูงแค่ 20px → เพิ่มเป็น 44px
