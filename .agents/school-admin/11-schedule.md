...
- **2026-09-23 SC-S10d + SC-S10e 🟢 verified** — agy เขียน 5 shard (API ×2 ขนาน · UI ×3 ขนาน) · Claude ตรวจเองทุกข้อ
  ...
  **การตัดสินใจ 4 ข้อ — 🔴 เจ้าของโปรเจคเคาะแล้ว 2026-09-23 · ห้ามเปลี่ยนเองในรอบหน้า:**
  (ก) ข้อยกเว้นมี **3 ประเภท** (งดคาบ/สอนแทน/ย้ายห้อง) ·
  (ข) **1 คาบ × 1 วันที่ = 1 รายการ** (unique · เปลี่ยน = แก้/ลบรายการเดิม) ·
  (ค) ครูสอนแทนที่ไม่ว่าง = **บล็อก 422** ไม่ใช่เตือน (คาบตัวเองที่ถูกงด/ถูกคนอื่นสอนแทนวันนั้น = ว่าง) ·
  (ง) ภาระงานสอน **นับทุก entry_type ยกเว้น break** · ครูที่มีคาบแต่ไม่อยู่ในทะเบียนครู (เช่น ผอ.) ขึ้นด้วย `is_member_teacher=false`
  ...
  **ตรวจบนเบราว์เซอร์จริงที่ 375px (dev server + JWT):** ภาระงานสอนขึ้น 120 ครู (119 ยังไม่มีคาบ — ตรงกับ DB) ไม่เลื่อนแนวนอน ·
  ...
  ✅ **แก้แล้ว 2026-09-23 (commit ตามหลัง):** `today()` เลือกภาคเรียนจากวันที่ที่ถามผ่าน `Semester::forAcademyOnDate()`
  (ครอบ start_date..end_date · fallback = ภาคเรียนปัจจุบันเมื่อวันนั้นนอกทุกภาคเรียน ⇒ แดชบอร์ด "วันนี้" ไม่เปลี่ยน)
  · ตรวจจริง: date ในภาคเรียนที่ 2 (2/2569) โชว์คาบของภาคเรียนนั้น ไม่ใช่ภาคเรียนปัจจุบัน · เทสต์ `test_today_resolves_semester_from_the_requested_date`
  ✅ **ตรวจ dashboard ครูบนจอจริงแล้ว 2026-09-24:** เจอบั๊ก `fetchStats()` เขียนทับ `stats.classesToday` ด้วย
  `classes_today` ของ backend (ไม่รู้เรื่องสอนแทน) รันขนานกับ `fetchTodaySchedule()` ใน Promise.all ⇒ ป้ายขึ้นถูกแต่ตัวนับผิด
  · แก้: ลบการเขียนทับใน fetchStats ให้ fetchTodaySchedule เป็นเจ้าของ classesToday คนเดียว
  · ยืนยัน: ครูสอนแทนเห็น "คุณสอนแทน" + นับ 1 · ครูประจำคาบเห็น "…สอนแทน" + นับ 0 (ไม่ต้องไปสอน)
