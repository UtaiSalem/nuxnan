...
| **E-S9c** | `[id].vue` + แท็บ ภาพรวม/พรรค/บัญชีผู้มีสิทธิ์ **+ งานค้างจาก E-S9b** (ดู §11.10) | codex | 🟡 **ตรวจ 2026-08-23 — backend ผ่าน · หน้าจอได้แค่โครง** (`aef62f32` + `9c2af0df`) → ต่อที่ E-S9c2 |
| **E-S9c2** | **ทำ 3 แท็บให้ใช้งานได้จริง** ตาม §11.11 | E-S9c | FE + BE | 🟡 **ตรวจ 2026-08-23 — ผ่าน 8 จาก 10 ข้อ** (`54b087f9` + `3ca3ba90`) · F1/F3/F4/F5/F6/F8/C2/C3/C5 ✅ · **F2 ยังผิดอยู่** → E-S9c3 |
| **E-S9c3** | **ปิด F2 + F7** — สรุปการล็อกต้องอ่านจาก `counts` ของ `lock()` ไม่ใช่ฟิลด์ที่ `stats` ไม่มี · คำเตือนต้องใช้ตัวเลขจริงไม่ใช่ค่าคงที่ · ช่องกรอกเบอร์ตอนอนุมัติ + `review_note`/`reviewed_at` · เทสต์ของ F3 | E-S9c2 | FE | ⚪ **งานถัดไป** |
| **E-S9d** | แท็บ หน่วย/ผล/บันทึก (+ G3 ฝั่งหน้า) | agy/codex | ⚪ |
