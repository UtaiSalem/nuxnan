...
| **E-S9d** | แท็บ หน่วย/ผล/บันทึก (+ G3 ฝั่งหน้า) | codex | 🟡 **ตรวจ 2026-08-23 — สามแท็บขึ้นครบแล้ว** (`a1e18db3`) · **แต่เส้นประกาศผลใช้จริงไม่ได้** ดู §11.13 → E-S9d2 |
| **E-S9d2** | 🔴 **`publish()` ไม่เคยเซ็ต `elections.published_at` ซึ่งเป็นสิ่งที่ `results()` ใช้เป็นด่าน** → `GET /results` ตอบ 404 ตลอดแม้ประกาศผลแล้ว · + URL ป้ายภาษาไทยผิด · + ผลตอนสถานะ `closed` ถูกทิ้ง | E-S9d | BE + FE | ⚪ **งานถัดไป** |
